...
internal class UserMapper : NewMapper {

    override fun toEntity(dto: Dto): User =
        if (dto is UserDto) convertToEntity(dto)
        else throw IllegalMappingArgumentException(expected = UserDto::class, received = dto)

    override fun toDto(entity: Entity): UserDto =
        if (entity is User) convertToDto(entity)
        else throw IllegalMappingArgumentException(expected = User::class, received = entity)
