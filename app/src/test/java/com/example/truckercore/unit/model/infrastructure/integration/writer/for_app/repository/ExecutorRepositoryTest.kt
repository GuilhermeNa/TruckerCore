...
import com.example.truckercore.model.infrastructure.integration.writer.for_app.repository.ExecutorRepository
import com.example.truckercore.model.infrastructure.integration.writer.for_app.repository.ExecutorRepositoryImpl
...
class ExecutorRepositoryTest : KoinTest {

    private val executor: InstructionExecutor<*> by inject()
    private val repository: ExecutorRepository by inject()
    ...
    fun setUp() {
        startKoin {
            modules(
                module {
                    single<InstructionExecutor<*>> { mockk() }
                    single { ExecutorAppErrorFactory() }
                    single<ExecutorRepository> { ExecutorRepositoryImpl(get(), get()) }
                }
            )
        }
    }
