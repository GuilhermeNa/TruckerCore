...
import com.example.truckercore.data.data_source.auth.impl.FirebaseAuthSourceImpl
import com.example.truckercore.data.infrastructure.data_source.auth.errors.abstractions.AuthSourceException
...
class AuthRepositoryImplTest : KoinTest {

    //----------------------------------------------------------------------------------------------
    // Setup
    //----------------------------------------------------------------------------------------------
    private val firebaseAuth: FirebaseAuth by inject()
    private val authSource: com.example.truckercore.data.data_source.auth.impl.FirebaseAuthSourceImpl by inject()
        ...
        startKoin {
            modules(
                module {
                    ...
                    single { com.example.truckercore.data.data_source.auth.impl.FirebaseAuthSourceErrorMapperImpl() }
                    single {
    ...
    companion object {
        ...
        fun provideFirebaseExceptionsForCreateUser() = arrayOf(
            Arguments.of(
                ...
                com.example.truckercore.data.data_source.auth.errors.auth_source_exceptions.WeakPasswordException::class
            ),
            Arguments.of(
                FirebaseAuthInvalidCredentialsException("auth/error", "invalid creds"),
                com.example.truckercore.layers.data.data_source.auth.errors.auth_source_exceptions.InvalidCredentialsException::class
            ),
            Arguments.of(
        ...
        fun provideFirebaseExceptionsForSendEmail() = arrayOf(
            Arguments.of(
                ...
                com.example.truckercore.data.data_source.auth.errors.auth_source_exceptions.SessionInactiveException::class
            ),
            Arguments.of(
    ...
    fun `observeEmailValidation should throw SessionInactiveException when user is null`() = runTest {
        ...
        // Act & Assert
        assertThrows<com.example.truckercore.data.data_source.auth.errors.auth_source_exceptions.SessionInactiveException> {
            authSource.observeEmailValidation()
        }
    }
