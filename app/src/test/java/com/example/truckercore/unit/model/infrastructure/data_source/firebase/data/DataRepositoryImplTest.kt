...
import com.example.truckercore.data.infrastructure.data_source.firebase.data.FirestoreSpecInterpreter
...
class DataRepositoryImplTest : KoinTest {

    //----------------------------------------------------------------------------------------------
    // Setup
    //----------------------------------------------------------------------------------------------
    private val interpreter: FirestoreSpecInterpreter by inject()
    private val dataSource: FirestoreDataSourceImpl by inject()
        ...
        startKoin {
            modules(
                module {
                    ...
                    single { FirestoreDataSourceErrorMapperImpl() }
                    single { FirestoreDataSourceImpl(get(), get()) }
                }
            )
        }
    }
    ...
    fun `findById should throw InvalidDataException when invalid data is encountered`() = runTest {
        ...
        every { interpreter.interpretIdSearch(any()) } throws com.example.truckercore.data.data_source.data.errors.data_source_exceptions.InvalidDataException()

        val exception = assertThrows<com.example.truckercore.data.data_source.data.errors.data_source_exceptions.InvalidDataException> {
            dataSource.findById(spec)
        }
    ...
    fun `findAllBy should throw InvalidDataException when invalid data is encountered`() = runTest {
        ...
        every { interpreter.interpretFilterSearch(any()) } throws com.example.truckercore.data.data_source.data.errors.data_source_exceptions.InvalidDataException()
