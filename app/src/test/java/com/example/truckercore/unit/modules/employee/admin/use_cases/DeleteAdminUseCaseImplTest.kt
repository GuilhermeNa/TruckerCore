...
class DeleteAdminUseCaseImplTest {
    ...
    private lateinit var useCase: DeleteAdminUseCase
    ...
    @Test
    fun `should delete entity when have permission and data exists`() = runTest {
        // Arrange
        every { permissionService.canPerformAction(user, Permission.DELETE_ADMIN) } returns true
    ...
    @Test
    fun `should return error when when entity does not exist`() = runTest {
        // Arrange
        every { permissionService.canPerformAction(user, Permission.DELETE_ADMIN) } returns true
